...
import org.onem2m.abstdev.constant.AppName;
import org.onem2m.abstdev.constant.ContactPort;
import org.onem2m.abstdev.constant.NativeLib;
import org.onem2m.abstdev.constant.ObjPath;
import org.onem2m.abstdev.constant.WellKnownName;
...
public class HumidifierClient {
	...
	private static IHumidifier mHumidifier;

	private static boolean isJoined = false;

	static class MyBusListener extends BusListener {
		public void foundAdvertisedName(String name, short transport, String namePrefix) {
			System.out.println(String.format("BusListener.findAdvertisedName(%s,%d,%s)", name, transport, namePrefix));
			...
			mProxyObj = mBus.getProxyBusObject(WellKnownName.ABST_DEV, ObjPath.HUMIDIFIER, sessionId.value, new Class<?>[] { IHumidifier.class });
	...
	public static void main(String[] args) {
		mBus = new BusAttachment(AppName.HUMIDIFIER, BusAttachment.RemoteMessage.Receive);
		...
		try {
			...
			System.out.println(mHumidifier.getHumidifier());
			System.out.println(mHumidifier.setHumidifier(18));
