...
#include <stdlib.h>
#include <stdint.h>
#include "org_onem2m_abstdev_impl_TempAndRHImpl.h"
 ...
 * Signature: ()I
 */
JNIEXPORT jint JNICALL Java_org_onem2m_abstdev_impl_TempAndRHImpl_getTempNative(JNIEnv *env, jobject obj)
{
    ...
    while(attempts){                        //you have 5 times to retry
        res = dht11_read_val();     //get result including printing out
        if (res!=NULL) {                      //if get result, quit program; if not, retry 5 times then quit
            break;
        }
        attempts--;
        delay(2500);
 ...
 * Signature: ()I
 */
JNIEXPORT jint JNICALL Java_org_onem2m_abstdev_impl_TempAndRHImpl_getRHNative(JNIEnv *env, jobject obj)
{
    int attempts=ATTEMPTS;
    if(wiringPiSetup()==-1)
        exit(1);
